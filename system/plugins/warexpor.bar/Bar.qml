...
Item {
  ...
  property color foreground: useAdaptiveInk
    ...
    : themeForeground
  property color barForeground: useAdaptiveInk
    ? adaptiveDrawForeground
  ...
  component ModuleSlot: Item {
    ...
    function injectProps() {
      ...
      if (!target) return
      // Stock first-party bar exposes pluginShellForId so third-party widgets get a
      // service-capable facade. Replacement bars (warexpor.bar) only get
      // pluginShellForBarEntry, which is service-less — media then can't see MPRIS.
      // Fall back to the bar root so bar.shell.firstPartyServiceFor keeps working.
      if ("bar" in target) {
        var canHostPluginShell = root.shell
          && typeof root.shell.pluginShellForId === "function"
        target.bar = (firstParty || !canHostPluginShell)
          ? root
          : root.pluginBarApiFor(pluginApiId, moduleName, registered)
      }
      // Clones / command modules often pin moduleName and settings as read-only.
      if ("moduleName" in target) {
        try { target.moduleName = moduleName } catch (e) {}
      }
      if ("settings" in target) {
        try { target.settings = moduleSettings } catch (e) {}
      }
    }
