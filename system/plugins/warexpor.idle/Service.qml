...
Item {
  ...
  readonly property string customScreensaverLauncher: home + "/.config/omarchy/bin/omarchy-launch-screensaver"
  ...
  function launchScreensaver() {
    ...
    // Prefer the 43PR launcher when the desktop bundle installed it, otherwise
    // fall back to Omarchy's stock launcher so this plugin also works alone.
    runProcess(screensaverProcess, "screensaver", "[[ $(omarchy-shell lock isLocked 2>/dev/null) == \"true\" ]] || { launcher=" + shellQuote(root.customScreensaverLauncher) + "; [[ -x \"$launcher\" ]] || launcher=omarchy-launch-screensaver; \"$launcher\"; }")
  }
  ...
  function lockSystem(reason) {
    ...
    // skips the custom runner trap that clears bar-off. Reclaim around lock
    // when the 43PR launcher is installed; standalone installs use stock lock.
    var restore = "[[ -x " + shellQuote(root.customScreensaverLauncher) + " ]] && " + shellQuote(root.customScreensaverLauncher) + " restore-bar || true"
    runProcess(lockProcess, "lock", restore + "; omarchy-system-lock; " + restore)
  }
  ...
  function cancelIdleCycle(reason) {
    ...
    if (root.idledThisCycle) {
      var restore = "[[ -x " + shellQuote(root.customScreensaverLauncher) + " ]] && " + shellQuote(root.customScreensaverLauncher) + " restore-bar || true"
      runProcess(wakeProcess, "wake", restore + "; omarchy-system-wake")
    }
