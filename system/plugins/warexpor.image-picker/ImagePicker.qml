...
Item {
  ...
  property string pluginDir: Quickshell.env("HOME") + "/.config/omarchy/plugins/warexpor.image-picker"
  ...
  // When false, carousel geometry snaps (open / filter jump). Arrow/click
  // selection keeps this true so x/width/height/y Behaviors run.
  property bool selectionAnimated: true
  property int selectionAnimMs: 220
  ...
  function select(index, immediate) {
    ...
    if (immediate === true) {
      ...
      selectedIndex = index
      Qt.callLater(function() { root.selectionAnimated = true })
      return
    }
        ...
        Item {
          ...
          Repeater {
            ...
            delegate: Item {
              ...
              Behavior on x {
                enabled: root.layoutSettled && root.selectionAnimated
                NumberAnimation { duration: root.selectionAnimMs; easing.type: Easing.OutCubic }
              }
              Behavior on y {
                enabled: root.layoutSettled && root.selectionAnimated
                NumberAnimation { duration: root.selectionAnimMs; easing.type: Easing.OutCubic }
